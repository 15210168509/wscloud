...
class DeviceController extends AdvancedRestController
{
    ...
    public function getDeviceSettingInfo()
    {
        if (!empty($GLOBALS['HTTP_RAW_POST_DATA'])) {
            ...
            $res = JPushTool::getInstance()->sendMessageByAlias($data['serialNo'],'消息','获取设备信息',$data);
            if ($res) {
    ...
    public function ajaxSaveDeviceSetting()
    {
        if (!empty($GLOBALS['HTTP_RAW_POST_DATA'])) {
            $data = json_decode($GLOBALS['HTTP_RAW_POST_DATA'], true);
            //推送到设备
            $res = JPushTool::getInstance()->sendMessageByAlias($data['serialNo'],'消息','设备配置信息',$data['data']);
            if ($res) {
                $this->setReturnVal(Code::OK,Msg::SET_SUCCESS,StatusCode::SET_SUCCESS);
            } else {
                $this->setReturnVal(Code::ERROR,Msg::SET_ERROR,StatusCode::SET_ERROR);
    ...
    public function restartDevice($serialNo)
    {
        ...
        $res = JPushTool::getInstance()->sendMessageByAlias($serialNo,'消息','设备重启',$data);
        if ($res) {
            $this->setReturnVal(Code::OK,Msg::SET_SUCCESS,StatusCode::SET_SUCCESS);
        } else {
            $this->setReturnVal(Code::ERROR,Msg::SET_ERROR,StatusCode::SET_ERROR);
    ...
    public function updateDevice($serialNo)
    {
        ...
        $res = JPushTool::getInstance()->sendMessageByAlias($serialNo,'消息','设备升级',$data);
        if ($res) {
            $this->setReturnVal(Code::OK,Msg::SET_SUCCESS,StatusCode::SET_SUCCESS);
        } else {
            $this->setReturnVal(Code::ERROR,Msg::SET_ERROR,StatusCode::SET_ERROR);
    ...
    public function checkVehicleDeviceInstallInfo()
    {
        if (!empty($GLOBALS['HTTP_RAW_POST_DATA'])) {
            ...
            $res = JPushTool::getInstance()->sendMessageByAlias($pare['serialNo'],'消息','检查车辆设备安装情况',$data);
            if ($res) {
                $this->setReturnVal(Code::OK,Msg::SET_SUCCESS,StatusCode::SET_SUCCESS);
            } else {
                $this->setReturnVal(Code::ERROR,Msg::SET_ERROR,StatusCode::SET_ERROR);
    ...
    {
        $data['type'] =  $type;
        $res = JPushTool::getInstance()->sendMessageByAlias($serialNo,'消息','消息',$data);
        if ($res) {
            $this->setReturnVal(Code::OK,Msg::SET_SUCCESS,StatusCode::SET_SUCCESS);
        } else {
            $this->setReturnVal(Code::ERROR,Msg::SET_ERROR,StatusCode::SET_ERROR);
